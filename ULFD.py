...
class UltraLightFaceDetection:
    def __init__(self, filepath, input_size=(320, 240),
                 conf_threshold=0.6, center_variance=0.1, size_variance=0.2, nms_max_output_size=200,
                 nms_iou_threshold=0.3) -> None:
    ...
    def __call__(self, img):
        cords = None
        face_locations, scores = self.inference(img)
        if len(face_locations) == 0:
            print(f"no face detected in target image, canceling")
            return None, None
        elif len(face_locations) > 1:
            print(f"multiple faces detected in target image, selecting the face the closest to the center")
            center = (img.shape[1] // 2, img.shape[0] // 2)
            distances = []
            for face_loc in face_locations:
                x, y, w, h = face_loc[0], face_loc[1], face_loc[2] - face_loc[0], face_loc[3] - face_loc[1]
                face_center = (x + w // 2, y + h // 2)
                distances.append(np.linalg.norm(np.array(face_center) - np.array(center)))
            index = np.argmin(distances)
            face_locations = [face_locations[index]]
            cords = (int(face_locations[0][0]), int(face_locations[0][1]),
                     int(face_locations[0][2]), int(face_locations[0][3]))
            scores = [scores[index]]
            return cords, scores
        for index_face, face_loc in enumerate(face_locations):
            x, y, w, h = face_loc[0], face_loc[1], face_loc[2] - face_loc[0], face_loc[3] - face_loc[1]
            cords = (int(x), int(y), int(x + w), int(y + h))
        return cords, scores
...
def ULFD_face_detector():
    return UltraLightFaceDetection(filepath="saved_models/version-slim-320_without_postprocessing.tflite")
